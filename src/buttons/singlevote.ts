...
module.exports = new Button(
    new ButtonBuilder()
        .setCustomId('singlevote')
        .setEmoji({ name: '🗳️' })
        .setStyle(ButtonStyle.Secondary),
    /*
         DESC: Toggles single vote mode for the associated poll. 
          PRE: The poll exists and is valid.
        PARAM: i - Interaction from button press.
         POST: The single vote is toggled and the poll post is updated.
    */
        ...
        try {
            ...
            if (poll.ended) {
                await i.reply(messageEmbed(
                    'You cannot toggle single vote for a poll that has already ended.'
                ) as InteractionReplyOptions);
                return;
            }

            // update singleVote for poll in db
            await client.mongo.db('Polls').collection<PollData>(i.guild.id).updateOne(
                { messageUrl: messageUrl },
                {
                    $set: { singleVote: !poll.singleVote }
                }
            );
